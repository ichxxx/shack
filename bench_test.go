...
func init() {
	...
	calcMem("ShackRouter", func() {
		...
		for _, route := range githubAPI {
			router.trie.insert(route.method, route.path, handler)
		}
		shackRouter = router
	})
...
// With GithubAPI
func BenchmarkShackRouterWithGithubAPI(b *testing.B) {
	benchRoutes(b, shackRouter, githubAPI)
}

func BenchmarkChiRouterWithGithubAPI(b *testing.B) {
	benchRoutes(b, chiRouter, githubAPI)
}

// With GithubAPI2
func BenchmarkShackRouterWithGithubAPI2(b *testing.B) {
	benchRoutes(b, shackRouter, githubAPI2)
}
